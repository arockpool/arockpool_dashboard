<template>
  <div class="dashboard-box">
    ...
    <div class="content">
      ...
      <div class="top-prominent">
        ...
        <div class="prominent-item">
          ...
          <div class="prom-con">
            ...
            <span class="prom-unit">
              {{
              pool_overview.total_power.split(" ")[1]
              }}
            </span>
          </div>
        </div>
        ...
        <div class="prominent-item">
          ...
          <div class="prom-con">
            ...
            <span class="prom-unit">
              {{
              pool_overview.increase_power.split(" ")[1]
              }}
            </span>
          </div>
        </div>
      ...
      <div class="big-box">
        <div class="left-box">
          <div class="mt-2 lt-center">
            <div class="rt-name">
              <span class="echarts-title">云池存储节点列表（共{{ miners_count }}个）</span>
            </div>
            <div style="padding-left: 2rem; width: 100%">
              ...
              >
                <li class="grid grid-flow-col grid-cols-4 grid-rows-1 gap-2 mb-3">
                  <div>存储节点</div>
                  <div>有效算力</div>
                  <div>24h算力增长</div>
                  <div>扇区大小</div>
                ...
                >
                  <div class="py-1">{{ item.miner_address }}</div>
                  <div class="py-1">{{ item.power }}</div>
                  <div class="py-1">{{ item.increase_power }}</div>
                  <div class="py-1">{{ item.sector_size_str.replace(/.00/, "") }}</div>
                </li>
              </ul>
            </div>
          </div>
    ...
    itemStyle: {
      normal: {
        color: function (v) {
          ...
          if (
            v.value[0] === _this.showInfo.lineS[0] &&
            v.value[1] === _this.showInfo.lineS[1]
          ) {
            color = "#FFCC00";
          }
          return color;
...
const setAreaNodes = (_this) => {
  ...
  setInterval(function () {
    ...
    if (sec >= the_sec) {
      pool_last_block_info.miner_address =
        area_nodes[Math.floor(Math.random() * area_nodes.length)];
...
export default {
  ...
  async asyncData({ app }) {
    ...
    if (overview.code === 0) {
      overview.data.total_reward = new BigNumber(
        new BigNumber(overview.data.total_reward).toFixed(2, 1)
    ...
    if (trend.code === 0) {
      ...
      Object.keys(pool_trend).forEach((key) => {
        let tib = (pool_trend[key].increase_power / 1099511627776)
          .toString()
          .split(".");
        sum_data[key] = Number(
          `${tib[0]}${typeof tib[1] !== "undefined" ? `.${tib[1].substring(0, 4)}` : ""
          }`
        );

        let tib1 = (pool_trend[key].power / 1099511627776 / 1024)
          .toString()
          .split(".");
        miner[key] = Number(
          `${tib1[0]}${typeof tib1[1] !== "undefined" ? `.${tib1[1].substring(0, 4)}` : ""
          }`
        );
      });
    ...
    let all = await Promise.all([
      app.$axios.post("/api/get_pool_miners", {
        sector_type: 0,
        page_size: 100,
      }),
      app.$axios.post("/api/get_pool_miners", {
    ...
    console.log(
      all[1].data.total_count,
      all[0].data.total_count,
      miners64.length,
      miners32.length,
      "okook"
    );
    let miners_all = [...miners64, ...miners32];
    let miners_show = miners_all.slice(0, 10);
    let miners_count = all[0].data.total_count + all[1].data.total_count;
  ...
  methods: {
    ...
    oneDay() {
      ...
      setInterval(function () {
        _this.$axios
          .post("/api/get_pool_trend", { days: 14 })
          .then((trend) => {
            if (trend.code === 0) {
              ...
              Object.keys(pool_trend).forEach((key) => {
                ...
                let tib1 = (pool_trend[key].power / 1099511627776 / 1024)
                  ...
                  .split(".");
                miner[key] = Number(
                  `${tib1[0]}${typeof tib1[1] !== "undefined"
                    ? `.${tib1[1].substring(0, 4)}`
                    : ""
                  }`
                );
              });
  ...
  mounted() {
    ...
    // rbRCon(this);
    let mKey = 0;
    _this.showPointer = [_this.mapJson[mKey].lineS, _this.mapJson[mKey].lineE]; // [data[1].lineS,data[1].lineE]
    _this.showInfo = {
      ..._this.mapJson[mKey],
    };
    loadChart(this);
